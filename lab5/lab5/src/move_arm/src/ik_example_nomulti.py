...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        request.ik_request.pose_stamped.pose.position.x = pose[0][3]
        request.ik_request.pose_stamped.pose.position.y = pose[1][3]
        request.ik_request.pose_stamped.pose.position.z = pose[2][3]
...
def stereo_process(outshm, mtx, b, lmlist1, lmlist2):
    """
...
def updateHandTrack(capid: int, shm, shm3d, mtx, dist, newcameramtx, roi, imshow=True):
    """
...
def hand_info():
    ...
    try:
        ...
        capture1 = updateHandTrack(cap1, lm1_q, lm4_q, mtx, dist, newcameramtx, roi)
        capture2 = updateHandTrack(cap2, lm2_q, lm5_q, mtx, dist, newcameramtx, roi)
        lm_to_3d = stereo_process(lm3d_q, mtx, b)
        orthoframe = find_orthonormal_frame(pose, True)
